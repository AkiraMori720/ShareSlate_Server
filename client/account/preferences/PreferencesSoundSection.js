...
const PreferencesSoundSection = ({ onChange, ...props }) => {
	...
	const settings = {
		...
		callNotification: useUserPreference('callNotification'),
	...
	return <Accordion.Item title={t('Sound')} {...props}>
		<FieldGroup>
			...
			{useMemo(() => <Field>
				<Field.Label>{t('Call_Notification')}</Field.Label>
				<Field.Row>
					<Select value={callNotification} onChange={handleCallNotification} options={soundsList} />
				</Field.Row>
			</Field>, [handleCallNotification, callNotification, soundsList, t])}
