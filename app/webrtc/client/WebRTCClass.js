...
class WebRTCTransportClass extends EventEmitter {
	...
	startCall(data) {
		...
		Meteor.call('sendCallNotificationToMySQL', { rid: this.webrtcInstance.room, userId: this.webrtcInstance.selfId, data: data }, function(error) {
   ...
   */
	...
	onRemoteOffer(data, ...args) {
		...
		const onAnswer = (answer) => {
			const onLocalDescription = () => {
				this.transport.sendDescripDescriptiontion({
					to: data.from,
					type: 'answer',
					ts: peerConnection.createdAt,
					description: {
