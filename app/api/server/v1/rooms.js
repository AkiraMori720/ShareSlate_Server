...
API.v1.addRoute('rooms.direct.get', { authRequired: true }, {
	get() {
		const { c_id } = this.queryParams;

		let otherUserId;
		if (c_id) {
			let otherUser = Users.findOneByCustomId(c_id, {fields:{_id: 1}});
			if(otherUser){
		...
		let result;
		Meteor.runAsUser(this.userId, () => { result = Meteor.call('rooms/direct/get', otherUserId); });
