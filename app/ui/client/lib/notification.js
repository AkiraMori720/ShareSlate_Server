	...
	newMessage(payload) {
		...
		if (!Session.equals(`user_${ Meteor.user().username }_status`, 'busy')) {
			...
			if (sub && sub.audioNotificationValue !== 'none') {
				console.log('notification sound', payload, sub, newMessageNotification);
				if (message.t && message.t === 'jitsi_call_started'){
					CustomSounds.play('ring', {
						volume: Number((audioVolume / 100).toPrecision(2)),
					});
				} else if (sub && sub.audioNotificationValue && sub.audioNotificationValue !== '0') {
