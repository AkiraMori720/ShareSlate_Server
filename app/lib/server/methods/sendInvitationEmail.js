...
Meteor.methods({
	...
	sendInvitationEmailWithMessage(emails, message) {
		check(emails, [String]);
		if (!Meteor.userId()) {
			throw new Meteor.Error('error-invalid-user', 'Invalid user', {
				method: 'sendInvitationEmailWithMessage',
			});
		}

		const validEmails = emails.filter(Mailer.checkAddressFormat);
		const user = Meteor.user();
		const subject = `You have been invited to ${settings.get('Site_Name')} by ${user.username}`;

		const invite_html = `<h4>${message}</h4><br/>` + html;
