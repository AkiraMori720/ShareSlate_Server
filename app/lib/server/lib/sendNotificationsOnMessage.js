...
import { shouldNotifyMySQL, notifyMessageToMySQL, notifyBroadcastToMySQL } from "/app/lib/server/functions/notifications/mysql";
...
export async function sendBroadcastNotificationToMySQL(room, user, type){
		if(settings.get('MySQL_Log')){
			console.log('sendBroadcastNotificationToMySQL:', type, room, user);
		}
		switch (type) {
			case 'create_channel':
				notifyBroadcastToMySQL({
					userId: user._id,
					messageType: type,
					room: room,
					notificationMessage:'You Added in new Group'
				});
				break;
			case 'add_user':
				// Direct Room Notification Disable
				if(room.t === 'd'){
					return;
				}
				const query = {
				...
				subscriptions.forEach((subscription) => {
					let message = `${user.username} Added in ${room.name} Group`;
					if(user._id === subscription.u._id){
						message = `You Added in ${room.name} Group`;
					}
					notifyBroadcastToMySQL({
						userId: subscription.u._id,
						messageType: type,
