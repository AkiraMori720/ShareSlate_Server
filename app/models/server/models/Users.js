...
export class Users extends Base {
	...
	findByEmailAddresses(emailAddresses, options) {
		const emails = emailAddresses.map(email => { return String(email).trim().toLowerCase(); });
		const query = { 'emails.address': { $in: emails }};
