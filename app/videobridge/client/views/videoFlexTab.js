...
import {Jitsi} from "/app/videobridge/client/JitsiClass";
...
Template.videoFlexTab.onRendered(function() {
	...
	const stop = () => {
		...
		if(Jitsi.getInstanceByRoomId(Session.get('openedRoom'))){
			Jitsi.getInstanceByRoomId(Session.get('openedRoom')).isStartedCall.set(false);
		}
	};
	...
	const onAcceptCall = (accept = false) => {
			...
			if (typeof JitsiMeetExternalAPI !== 'undefined') {
				...
				if (!$('[id^=jitsiConference]').length) {
					Tracker.nonreactive(async () => {
						...
						if(!accept){
							Jitsi.getInstanceByRoomId(Session.get('openedRoom')).startCall({});
						}
						/*
	...
	if(Jitsi.getInstanceByRoomId(Session.get('openedRoom')).isStartedCall.get()){
		return onAcceptCall(true);
	}
	...
	modal.open({
		...
		Meteor.call('sendCallNotificationToMySQL', { rid, userId: Meteor.userId() }, function(error) {
